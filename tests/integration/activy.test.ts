...
describe("GET /activy", () => {
  it("should respond with status 401 if no token is given", async () => {
    const response = await server.get("/activy");
  ...
  it("should respond with status 401 if given token is not valid", async () => {
    ...
    const response = await server.get("/activy").set("Authorization", `Bearer ${token}`);
  ...
  describe("when token is valid", () => {
    ...
    describe("when ticket is valid", () => {
      it("should respond with status 200 and an empty array when there are no activy date created", async () => {
        const user = await createUser();
        const token = await generateValidToken(user);
        const enrollment = await createEnrollmentWithAddress(user);
        const ticketType = await createTicketTypeWithOrWithoutHotel();
      ...
      it("should respond with status 200 and with activy date data", async () => {
...
describe("GET /activy/:eventDateId", () => {
  it("should respond with status 401 if no token is given", async () => {
    const response = await server.get("/activy/1");
  ...
  it("should respond with status 401 if given token is not valid", async () => {
    ...
    const response = await server.get("/activy/1").set("Authorization", `Bearer ${token}`);
  ...
  describe("when token is valid", () => {
    ...
    describe("when ticket is valid", () => {
      it("should respond with status 400 when param eventDateId is invalid", async () => {
        ...
        const response = await server.get("/activy/0").set("Authorization", `Bearer ${token}`);
      ...
      it("should respond with status 200 and an empty array of events when there are no events created", async () => {
        ...
        const response = await server.get(`/activy/${date.id}`).set("Authorization", `Bearer ${token}`);
